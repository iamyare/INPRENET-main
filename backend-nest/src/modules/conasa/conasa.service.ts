...
export class ConasaService {
  ...
  async buscarInformacionCompletaPorNombres(
    terminos: string,
    email: string,
    password: string
  ): Promise<any> {
    ...
    // Validar que haya al menos 2 términos
    const palabras = terminos.split(' ').map((palabra) => palabra.trim().toLowerCase());
    if (palabras.length < 2) {
      throw new BadRequestException('Debe proporcionar al menos dos términos de búsqueda.');
    }
